...
[Critical] Default LLM backend is **Ollama**, running locally, model `gemma`, connecting to the existing local Ollama server (`http://localhost:11434`). Do not re-pull or re-download the model — assume it is already present.
...
- Ollama endpoint (`http://localhost:11434` ) and model tag (`gemma` ) — already running locally
